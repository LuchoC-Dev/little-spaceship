...
- **Reviewer memory on `main` is usually ahead of the copy on a feature branch.** Phase branches are cut before the previous review's memory commit lands, so `.claude/agent-memory/reviewer/` in a phase worktree is the older revision. Writing there is still correct — memory belongs in the worktree being audited, and putting it in the main worktree once left a dirty `main` that had to be rescued — but write the *superset*: read `git show main:.claude/agent-memory/reviewer/<file>` first and add to that, so the merge cannot silently revert the newer text. Check with `git log --oneline --all -- .claude/agent-memory/reviewer/`.

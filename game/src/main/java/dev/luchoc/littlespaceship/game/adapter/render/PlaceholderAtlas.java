 ...
 *
 * <p><b>This is the fallback now, not the default.</b> {@link PackedSpriteAtlas#load} prefers the
 * real atlas generated by {@code docs/design/atlas/build-atlas.js} from the hand-drawn pixel art in
 * {@code docs/design/mockups/src/01-sprites.js}, and only falls back to this class when {@code
 * assets/atlas/sprites.atlas} is missing or a sprite id is not in either set. Kept rather than
 * deleted because a checkout that has not generated the atlas — or a future sprite id introduced by
 * {@code core} before art catches up — should still draw something legible instead of nothing.
 *

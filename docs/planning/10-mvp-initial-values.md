...
under ideal fire.** The mechanism the beat is built on needs the carrier to live 4.0 s, the
`Spawner` interval, before its first child. At 700 hp that means the player must land less than
175 damage per second on it. From the Roster's derived columns and the Drops table:

| beat | carrier appears | shot level a player who took every upgrade holds | ideal sustained dps | time to kill |
|---|---|---|---|---|
| `l1-heavy-carrier` | 67.0 s | 3 | 200 | **3.5 s — under 4.0 s** |
| `l1-twin-carriers-attachment` | 97.5 s | 4 | 333 | **2.1 s — under 4.0 s** |
...
shot level 4 needed 3.0 s. **Open, and the first thing the next play session should report: does the
carrier still spawn children?** If it does not, the fix is the carrier's health or the `Spawner`
interval, not the beat's length.

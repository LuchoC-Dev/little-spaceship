...
| **A flat timeline of all 261 spawn events at absolute times** | That is `level-01.json` as it stood before phase 11b — a transcript, not a design — and reintroducing it as "the document" would undo the phase that removed it. Section 3 aggregates per placement; section 5 gives local times per wave. Both are readable; 261 rows are not. |

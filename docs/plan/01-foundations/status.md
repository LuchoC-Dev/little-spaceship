...
- **What is still unverified:** that the generated stream is identical under TeaVM. The algorithm uses only operations with identical semantics in JavaScript, and multiplication is deliberately avoided, but nobody has run it in a browser yet. Phase 09 is where that gets confirmed; if it ever diverges, this is the first place to look.

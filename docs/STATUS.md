...
by them as correct. **The 11 group is complete and on `dev`** — 11f was the last of it, the four web defects. **11g is done and on `dev`** — the `shield` drop level 1 turned out to be missing, a third task the play session found, and the half of [#19](https://github.com/LuchoC-Dev/little-spaceship/issues/19) that was never blocked. **`game` has tests for the first time.** **Phase 11h is done and on `dev`** — a test mode, so one wave or the boss can be opened from a menu instead of playing 134.5 s to reach it, as a build flavour whose code is absent from the shipped build rather than hidden in it. **Phase 11i is done and on `dev`** — enemies can now follow a drawn path: enter, turn, wait, loop a bounded number of times, leave, with mirroring for free. **Phase 11j is next**: an absolute authoring syntax over the relative core, a speed multiplier, curves, and level 1's redesign — an agenda that came out of the project owner playing 11i rather than out of a plan. **Phase 12 is not planned yet.** **Phase 12 follows**: levels 2 and 3, which the roadmap calls the first honest measurement of whether this group worked.

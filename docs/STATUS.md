...
by them as correct. **The 11 group is complete and on `dev`** — 11f was the last of it, the four web defects. **11g is open**, a short phase the group left behind rather than one the roadmap planned: a `shield` drop for level 1, which 11f proved unreachable the moment it wired the icon, and the half of [#19](https://github.com/LuchoC-Dev/little-spaceship/issues/19) that was never blocked — a way to test `game` at all, open since phase 03. **Phase 12 follows**: levels 2 and 3, which the roadmap calls the first honest measurement of whether this group worked.

...
- `spikes/web-viability/` — a throwaway prototype that validated the platform. Not the base of the game. **Now deletable, and still present**: the exception that kept it — `rngcheck/` being the only measurement that `Rng` produces a bit-identical stream under TeaVM — ended on 27/08/2026 when phase 11a resolved [#52](https://github.com/LuchoC-Dev/little-spaceship/issues/52). The check now runs against the real `core` class in the `:rngparity` subproject (`./gradlew :rngparity:rngParityCheck`, `rngparity/src/main/java/dev/luchoc/littlespaceship/rngparity/Main.java`), with no copy of `Rng.java` anywhere. Deleting the directory is a separate act and the project owner's. Resolved [#5](https://github.com/LuchoC-Dev/little-spaceship/issues/5); see `docs/plan/10a-honest-documentation/decisions.md`, D1.

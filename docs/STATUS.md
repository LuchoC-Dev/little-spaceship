...
by them as correct. **The 11 group is complete and on `dev`** — 11f was the last of it, the four web defects. **11g is done and on `dev`** — the `shield` drop level 1 turned out to be missing, a third task the play session found, and the half of [#19](https://github.com/LuchoC-Dev/little-spaceship/issues/19) that was never blocked. **`game` has tests for the first time.** **Phase 12 is next** and is not planned yet. **Phase 12 follows**: levels 2 and 3, which the roadmap calls the first honest measurement of whether this group worked.

...
public final class ScoreSystem implements GameSystem {
     ...
     * {@code World.outcome()} (phase 06) can now detect {@code LevelOutcome.COMPLETED}, but nothing
     * yet calls this method to actually award the bonus when that happens — the boss and a real
     * victory condition are still phase 07's job, and crediting a bonus on today's "timeline ran
     * dry" signal risks awarding it under a rule phase 07 changes shortly after.
     *
     ...
     * exposes {@code PlayerStatus}, a read-only snapshot, not this. Making it public ahead of a real
     * caller would be exactly the ahead-of-need abstraction the project avoids, and would cross a
     * mutable domain type where the boundary requires read-only or immutable data. Whatever
     * eventually awards this bonus belongs in this same package and can call it directly; if it
     * turns out to need this from outside {@code domain.system}, that need is the point to design
     * the read-only shape, not now.
     *
